...
*** trnsfr - Transfer data from saf to daf.
     ...
     1          rec1m1,rec1m2,rec1m3,rec1m4,rec1g,rec1a)
      ...
      WRITE(wterm,*) ' starting bragg-williams transfer '
...
***          starting at REC=rec1.
      ...
      IMPLICIT DOUBLE PRECISION (a-h,o-z)
      DOUBLE PRECISION kappa, kappap, kappadp, numatoms, n
      PARAMETER (NTPLUS = 4)
...
*****      read data from saf
 ...
 90            FORMAT(6x, f8.2, 2x, f6.2, 2x, f7.4)
           ENDIF
           IF(ntran == 2) THEN
               READ(saf,100) dH, dV, W, Wv, n, SF
 100           FORMAT(6x, f6.2, 2x, f7.3, 2x, f6.2, 1x, f7.3, 
     1                1x, f6.2, 1x, f5.2)
           ENDIF
           READ(saf,80) Tmax
 80        FORMAT(8x,f7.2)
...
*****      write data to daf *****
     ...
     1       , numatoms
           ...
           ELSE IF(ntran == 2) THEN
               WRITE(daf,100,REC=r+6) dH, dV, W, Wv, n, SF
               WRITE(daf,80,REC=r+7) Tmax
           ELSE
